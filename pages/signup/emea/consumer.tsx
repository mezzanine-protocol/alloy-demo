...
import { Button, StyledLabel } from '../../../components/styles';
...
import { emeaConsumerFormFields } from '../../../formFields/emeaConsumer';
...
const UkConsumer = () => {
  ...
  const { productName, companyName, setEntityToken } =
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const apiResponse = await fetch(
        `/api/applications/${journeyApplicationToken}`,
        {
          method: 'GET',
          headers: {
  ...
  }, [stage, response, apiCalls, setApiCalls]);
  ...
  const handleCreateApplication = async (data: IFormFields) => {
    ...
    setResponse(responseJson);
    setEntityToken && setEntityToken(externalEntityId);
  };
          ...
          <AnimatePresence>
            {stage === 'initial' && (
              ...
              >
                ...
                >
                  {emeaConsumerFormFields.map((fieldSection) => {
                    return (
                      <Box
                        key={fieldSection.id}
                        css={{
...
UkConsumer.getLayout = function getLayout(page: ReactElement) {
  return <FormLayout>{page}</FormLayout>;
};

export default UkConsumer;
